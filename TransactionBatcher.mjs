...
export class TransactionBatcher {
  ...
  validateTransactionInfo(txInfo) {
    try {
      new TransactionBuilder.validateTransactionInfo(txInfo);
    } catch (e) {
      return false;
    }
    return true;
        ...
        const delayedSend = async () => {
          resolvedTransactions[index] = await batchedTransaction.then(
            (transaction) => {
              ...
              transaction.processTransaction(senderSk).then(async (result) => {
                console.log(`tx#${index + 1} processed`);
                return await transaction.checkTranscationResult(result);
                /**
                      .then((processed_block) => {
                      if (processed_block) {
